...
export default function AdminPanel({
  ...
  useEffect(() => {
    ...
    getUsuarios()
      .then((lista) => {
        if (!cancelado) setUsers(lista.map(mapUser))
      })
      .catch(fallo)
    getContratos()
      .then((lista) => {
    ...
    getFormatos()
      .then((lista) => {
        if (!cancelado) setFormatos(lista)
      })
      .catch(fallo)
    getFirmas()
      .then((lista) => {
        ...
        {tab === 'dashboard' && (
          <>
            {errorDatos && (
              ...
              >
                No se pudieron consultar algunos datos del servidor. Las cifras de abajo pueden estar incompletas —
            ...
            >
              <Widget
                ...
                value={errorDatos ? '—' : String(contratosActivos)}
                hint={errorDatos ? 'Dato no disponible' : `${totalContratos} registrados en total`}
              />
              <Widget
              ...
              {formatos.length === 0 && (
                <div style={{ padding: '40px 16px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 4 }}>
                    Aún no hay formatos cargados
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    Utilice "+ Cargar formato" para cargar los formatos oficiales (PDF, DOCX o XLSX).
                  </div>
                </div>
              )}
              {formatos.map((f) => (
                <GridRow key={f.id} cols="140px 1fr 80px 150px 150px 250px">
                  ...
                  <span style={{ fontSize: 12 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                      {f.tipoArchivo} · {formatBytes(f.tamanioBytes)}
                      {f.subidoPorNombre ? ` · ${f.subidoPorNombre}` : ''}
                    </div>
                  </span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>{f.version}</span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                    {formatFecha(f.fechaActualizacion)}
                  </span>
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              {users.map((u) => (
                <GridRow key={u.id} cols="1fr 260px 150px 190px 110px 220px">
                  <span style={{ fontSize: 12 }}>
                    {u.nombre}
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Centros: {u.centros}</div>
                  </span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                    {u.correo}
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{u.telefono}</div>
                  </span>
                  ...
                  <span style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <MiniBtn
                      onClick={() => {
                        setFirmaUsuarioPreseleccionado(u)
                    ...
                    <MiniBtn
                      ...
                      title="Asignar una contraseña temporal nueva y enviarla al correo del usuario"
                    >
                      Resetear clave
                    </MiniBtn>
                    <MiniBtn onClick={() => toggleActivo(u)}>{u.activo ? 'Desactivar' : 'Reactivar'}</MiniBtn>
                  </span>
                </GridRow>
              ))}
            </div>
            ...
            <SectionHead
              ...
              action={
                <button
                  className="btn-green"
                  onClick={() => {
                    setFirmaUsuarioPreseleccionado(null)
                    setFirmaModalOpen(true)
                  }}
                  style={{ padding: '8px 14px', fontSize: 12 }}
                >
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              {firmas.length === 0 && (
                <div style={{ padding: '40px 16px', textAlign: 'center' }}>
                  <IconSignature
                    size={26}
                    style={{ opacity: 0.4, margin: '0 auto 10px', color: 'var(--text-muted)' }}
                  />
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 4 }}>
                    Aún no hay firmas asignadas
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    Utilice "+ Asignar firma" o el botón "Asignar firma" en Gestión de Usuarios.
                  </div>
                </div>
              )}
              {firmas.map((f) => (
                <GridRow key={f.id} cols="1fr 260px 190px 190px 120px 160px">
                  ...
                  <span style={{ display: 'flex', gap: 6 }}>
                    <MiniBtn onClick={() => toggleFirma(f)}>{f.activa ? 'Revocar' : 'Restaurar'}</MiniBtn>
                  </span>
                </GridRow>
              ))}
            </div>
