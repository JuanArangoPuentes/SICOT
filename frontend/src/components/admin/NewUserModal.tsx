...
export function NewUserModal({ onClose, onCreate }: { onClose: () => void; onCreate: (u: UserRow) => void }) {
  ...
  const ROL_MAP: Record<string, Rol> = {
    ...
    'Gestor de Contratación': 'GESTION',
  ...
  const crear = async () => {
    ...
    if (!correo.endsWith('@soy.sena.edu.co')) {
      setError('El correo debe pertenecer al dominio @soy.sena.edu.co')
      return
    }
    if (!tel.trim()) {
        ...
        <div style={{ textAlign: 'center', padding: '10px 0 4px' }}>
          ...
          <div
            style={{
              ...
              fontSize: 12.5,
              textAlign: 'left',
              background: enviandoCredenciales
                ? 'var(--bg-surface)'
          ...
          >
            ...
            {!enviandoCredenciales && envioResultado && !envioResultado.enviado && (
              <>
                No fue posible enviar el correo: {envioResultado.error}. Copie la contraseña que aparece a continuación
                y compártala de forma manual.
              </>
            )}
          </div>
    ...
    <Modal title="Crear nuevo supervisor / gestor" onClose={onClose} width={520}>
      ...
      <Field label="Cargo">
        <select value={rol} onChange={(e) => setRol(e.target.value)}>
          ...
          <option>Gestor de Contratación</option>
          <option>Administrador</option>
        </select>
      </Field>
      <div className="card" style={{ padding: '12px 14px', margin: '4px 0 12px', borderColor: 'var(--accent-line)' }}>
        <div
          style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--accent)', marginBottom: 6 }}
        >
        ...
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
          Generada automáticamente · 12 caracteres · almacenada (cifrada) · se envía por correo institucional al crear
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button className="btn-ghost" style={{ flex: 1, padding: '10px 0', fontSize: 13 }} onClick={onClose}>
          Cancelar
        </button>
        <button
          className="btn-green"
