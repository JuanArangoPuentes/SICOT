...
export default function Bandeja({
  ...
  const pasoActivo =
    steps.find((s) => s.status === 'active') ?? steps.find((s) => s.subSteps.some((ss) => !ss.completed))
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '26px 24px 32px', minWidth: 0 }}>
      <div style={{ maxWidth: 940, margin: '0 auto' }}>
          ...
          {todoCerrado ? (
            ...
            <p style={{ margin: 0, fontSize: 14.5, color: 'var(--text-secondary)', lineHeight: 1.65, maxWidth: 680 }}>
              ...
              <strong style={{ color: 'var(--text-primary)', fontWeight: 600 }}>
                Paso {pasoActivo.id} de {steps.length}
              </strong>{' '}
              — {pasoActivo.title}.
            </p>
          ) : (
            ...
            <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
              <div className="surface" style={{ padding: '11px 14px', flex: '1 1 380px', minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13.5, color: 'var(--text-primary)', lineHeight: 1.5 }}>
                  <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent)', marginRight: 7 }}>
                    {subPasoActual.id}
                  </span>
                  {subPasoActual.label}
                </div>
              ...
              <button
                className="btn-green"
                onClick={() => onContinuarPaso(pasoActivo.id)}
                style={{
                  padding: '11px 20px',
                  fontSize: 13.5,
                  display: 'inline-flex',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
          ...
          <span
            style={{
              fontFamily: 'var(--font-mono)',
              fontSize: 12.5,
              color: criticos ? 'var(--alert-critica)' : 'var(--text-muted)',
            }}
          >
            {items.length === 0
              ? 'nada pendiente'
        ...
        {errorAlertas && (
          ...
          >
            <span className="inbox-icon" style={{ background: 'var(--chip-red-bg)', color: 'var(--alert-critica)' }}>
              !
            </span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--alert-critica)' }}>
                No se pudieron consultar las alertas del contrato
              </div>
              ...
              <div key={item.id} className={`inbox-row ${item.severidad}`}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 3 }}>
                    <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.11em', color: meta.color }}>
                      {meta.label}
                    </span>
                    <span
                      style={{
