  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: 24, minWidth: 0 }}>
      ...
      {alertaCronograma && (
        <AlertCard
          severity={
            alertaCronograma.severity === 'ok' ? 'ok' : alertaCronograma.severity === 'critica' ? 'urgent' : 'attention'
          }
      ...
      {errorAlertas ? (
        <div
          className="card"
          style={{ padding: '40px 20px', textAlign: 'center', borderColor: 'var(--alert-critica)' }}
        >
          <div
            style={{
              width: 34,
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--alert-critica)', marginBottom: 4 }}>
        ...
        alertasApi.filter((a) => !a.leida).length === 0 &&
        !alertaCronograma && (
          <div className="card" style={{ padding: '40px 20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              No hay alertas pendientes para este contrato.
            </div>
          </div>
        )
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '0.1em', color: config.color }}>
              {config.label}
            </span>
