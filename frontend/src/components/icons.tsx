...
export function IconLoader({ size = 28, style, className }: IconProps) {
  return (
    <svg
      className={className}
      style={{ ...base(size), animation: 'spin 0.9s linear infinite', ...style }}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
...
export function IconAvatarSena({ size = 24, style, className }: IconProps) {
  return (
    ...
    >
      ...
      <path
        d="M12 7.6l1.15 2.35 2.55.4-1.85 1.85.45 2.55L12 13.6l-2.3 1.15.45-2.55-1.85-1.85 2.55-.4Z"
        fill="currentColor"
        stroke="none"
      />
    </svg>
...
export function AvatarIcon({ id, size = 24, style, className }: IconProps & { id: string }) {
  ...
  switch (id) {
    case 'profesional':
      return <IconAvatarProfesional {...props} />
    case 'legal':
      return <IconAvatarLegal {...props} />
    case 'bot':
