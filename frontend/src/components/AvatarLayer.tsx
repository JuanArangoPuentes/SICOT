...
export default function AvatarLayer({
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      setPos({
        x: Math.min(e.clientX + 26, window.innerWidth - 96),
        y: Math.min(e.clientY + 26, window.innerHeight - 96),
      })
      clearTimeout(idle)
    ...
    return (
      <>
        ...
        >
          ...
          >
            <AvatarIcon id={prefs.avatarId} size={34} />
          </div>
          <div className="card" style={{ maxWidth: 300, padding: '14px 16px', borderColor: 'var(--accent-line)' }}>
            <div
              style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--accent)', marginBottom: 6 }}
            >
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              ...
              {step < tour.length - 1 ? (
                ...
                <button className="btn-green" style={{ padding: '6px 12px', fontSize: 12 }} onClick={onTourEnd}>
                  Completar tutorial ✓
                </button>
              )}
              <button
